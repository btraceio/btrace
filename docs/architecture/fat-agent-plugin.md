...
Maven fat-agent packaging is withdrawn for 3.0.0. The former goal resolved artifacts and staged
classdata according to pre-3.0 contracts, which could produce a successful build whose embedded
services could not load. The goal now fails explicitly. The Gradle plugin described in this guide
is the supported 3.0 fat-agent implementation.

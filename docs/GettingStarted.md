...
Maven fat-agent packaging is not supported in 3.0.0. The in-repository `fat-agent` goal fails with
an explicit unsupported message because its pre-3.0 artifact contract cannot safely load published
3.0 extensions. Use the [`io.btrace.fat-agent` Gradle plugin](../btrace-gradle-plugin/README.md),
or deploy the standard `io.btrace:btrace` artifact without embedded extensions.

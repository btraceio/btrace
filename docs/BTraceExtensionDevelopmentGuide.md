...
The Maven `fat-agent` goal is explicitly unsupported in 3.0.0 because it targets the pre-3.0
extension publication and classdata layout. Use the Gradle fat-agent plugin for embedded
extensions. Invoking the Maven goal fails rather than producing a partially functional agent.

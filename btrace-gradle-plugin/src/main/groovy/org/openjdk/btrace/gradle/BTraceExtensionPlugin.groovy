...
class BTraceExtensionPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
                ...
                doFirst {
                    ...
                    manifest {
                        attributes(
                            ...
                            'BTrace-Provided-Locator': extension.providedLocator ?: '',
                            'BTrace-Provided-Required': extension.providedRequired ? 'true' : '',
        ...
        // Generate locator properties file if provided deps are configured
        def locatorPropsDir = new File(project.buildDir, 'generated/resources/btraceLocator')
        def generateLocatorProps = project.tasks.register('generateLocatorProperties') {
            outputs.dir(locatorPropsDir)
            doLast {
                if (extension.providedLocator) {
                    def propsDir = new File(locatorPropsDir, 'META-INF')
                    propsDir.mkdirs()
                    def propsFile = new File(propsDir, 'btrace-extension.properties')
                    def props = new Properties()
                    extension.providedProperties.each { k, v ->
                        props.setProperty("locator.${k}", v)
                    }
                    propsFile.withWriter { w ->
                        props.store(w, "Generated by btrace-extension plugin for ${extension.id}")
                    }
                    project.logger.lifecycle("[BTRACE-EXT] Generated locator properties: ${extension.providedProperties.keySet()}")
                }
            }
        }
        ...
        buildApiJar.configure {
            dependsOn compileServiceShims, generateShimIndex, generateExportsIndex, generateLocatorProps
            from(shimsClsDir)
            from(shimsResDir)
            from(exportsResDir)
            from(locatorPropsDir)
        }
...
class BTraceExtensionMetadata {
    ...
    // Provided dependency locator configuration
    String providedLocator
    Map<String, String> providedProperties = [:]
    boolean providedRequired = false

    /**
     * Configure provided dependency locator.
     * <pre>
     * provided {
     *     locator = '[EnvVarLocator]'
     *     properties = ['envVar': 'SPARK_HOME', 'subdir': 'jars']
     *     required = true
     * }
     * </pre>
     */
    void provided(@DelegatesTo(ProvidedDepsSpec) Closure cl) {
        def spec = new ProvidedDepsSpec()
        cl.delegate = spec
        cl.resolveStrategy = Closure.DELEGATE_FIRST
        cl.call()
        this.providedLocator = spec.locator
        this.providedProperties = spec.properties
        this.providedRequired = spec.required
    }

    static class ProvidedDepsSpec {
        /** Locator class: use [ShortName] for built-in or FQN for custom. */
        String locator
        /** Locator configuration properties (keys without 'locator.' prefix). */
        Map<String, String> properties = [:]
        /** Whether deps are required (fail extension load if not found). */

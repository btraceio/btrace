 ...
 *
 * <p>Tracks vector database query performance, retrieval quality metrics, and end-to-end RAG
 * pipeline latency. Works with any vector DB client (Pinecone, Milvus, Weaviate, Chroma, pgvector,
 * Qdrant).
 *
...
public interface RagQualityService {
   ...
   *
   * <p>Use inline on the calling thread and call {@link QueryRecord#record()} before the next
   * {@code query()} call. Do not store the returned reference.
   *
   * <pre>
   * rag.query("pinecone")
   *     .resultCount(5)
   ...
   */
  @Nullable QueryRecord query(String source);
  ...
  /**
   * Records a chunk that was retrieved and used in context. Useful for tracking context window
   * utilization.
   *
   * @param source vector DB or index name
  ...
  @Nullable String getSourceSummary(String source);

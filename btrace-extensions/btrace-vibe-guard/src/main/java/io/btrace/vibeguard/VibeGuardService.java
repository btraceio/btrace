 ...
 *
 * <p>Validates that methods respect declared invariants at runtime — useful for "vibe coding"
 * workflows where LLMs generate code that needs guardrails. Tracks contract violations, latency
 * budgets, call frequency limits, and return value constraints without modifying the target code.
 *
 * <p>Contracts are defined in BTrace scripts and enforced at instrumentation points. When a
 * contract is violated, the service records it and optionally triggers an alert via the configured
 * handler.
 *
...
public interface VibeGuardService {
  ...
  /**
   * Checks that call rate does not exceed the limit per second. Uses a sliding window to detect
   * bursts.
   *
   * @param contract contract/method name
  ...
  /**
   * Tags a method invocation as AI-generated. Used to compare behavior between AI and human code
   * paths.
   *
   * @param contract contract/method name
  ...
  /**
   * Returns true if any contract has been violated. Useful in {@code @OnEvent} handlers for
   * alerting.
   */

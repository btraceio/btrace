...
public class VariableMapperTest {
  ...
  public void testMapUnmappedVariable() {
    ...
    InstrumentationException ex = assertThrows(InstrumentationException.class, () -> mapper.map(3));
    assertTrue(
        ex.getMessage().contains("not remapped") || ex.getMessage().contains("out of bounds"));
  }

...
public final class ExtensionRepository {
    ...
    private ExtensionRepository() {
        try {
            ...
            if (source == null) {
                String clzUrlStr = ClassLoader.getSystemResource(ExtensionRepository.class.getName().replace('.', '/') + ".class").toString();
                clzUrlStr = clzUrlStr.replace("jar:file:", "");
                int idx = clzUrlStr.lastIndexOf("!");
                source = new CodeSource(Paths.get(clzUrlStr.substring(0, idx)).toUri().toURL(), (CodeSigner[])null);
            }
            ...
            if (Files.exists(repoDir) && Files.isDirectory(repoDir)) {
                Files.walkFileTree(repoDir, new FileVisitor<Path>() {
                    @Override
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                        if (file.toString().toLowerCase().endsWith(".jar")) {
                            ...
                            ExtensionEntry current = extensionsById.computeIfAbsent(entry.getId(), k -> entry);
                            if (current != entry) {

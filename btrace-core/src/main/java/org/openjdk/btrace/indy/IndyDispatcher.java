...
public final class IndyDispatcher {
  ...
  private static final MethodHandle CHECK_LEVEL;
          ...
          MethodHandles.lookup()
              .findStatic(
                  ...
                  MethodType.methodType(
                      int.class, HandlerRepository.class, String.class, int.class, int.class));
   ...
   *
   * @param caller lookup context provided by the JVM (unused, reserved)
  ...
  public static CallSite bootstrap(
    ...
    try {
      ...
      if (repo == null) {
        // System.err is used intentionally: this class lives in the bootstrap classloader
        // where SLF4J may not yet be available.
        System.err.println("BTrace: IndyDispatcher.repository not initialized for " + probeClassName + "#" + name);
        return new ConstantCallSite(MethodHandles.dropArguments(NOOP, 0, type.parameterArray()));
      }
      MethodHandle mh;
      try {
  ...
  public static CallSite runtimeBootstrap(
    ...
    try {
      ...
      if (repo == null) {
        // System.err is used intentionally: this class lives in the bootstrap classloader
        // where SLF4J may not yet be available.
        System.err.println("BTrace: IndyDispatcher.repository not initialized for runtime " + owner + "#" + name);
        return new ConstantCallSite(buildDefaultHandle(type));
      }
      MethodHandle mh;
      try {
  ...
  /**
   * Bootstrap method for probe level checks. Replaces the former {@code GETSTATIC
   * probeName.$btrace$$level} instruction emitted into instrumented target classes. Returns a
   * {@link ConstantCallSite} whose target re-reads the probe level on every invocation so that
   * dynamic level changes via {@code setInstrumentationLevel} take effect immediately.
   *
   * @param caller lookup context (unused)
   * @param name unused call-site name
   * @param type expected to be {@code ()I}
   * @param probeName the fully qualified probe class name
   * @param lo the inclusive lower bound of the accepted level range
   * @param hi the inclusive upper bound of the accepted level range
   * @return a call site returning 1 if {@code lo <= probe level <= hi}, else 0
   */
  public static CallSite levelBootstrap(
    ...
    try {
      HandlerRepository repo = repository;
      if (repo == null) {
        return new ConstantCallSite(MethodHandles.constant(int.class, 0));
      }
      // Bind repo, probeName, lo, hi into checkLevel so each invocation re-reads the live level.
      // This mirrors the old GETSTATIC semantics: dynamic level changes are reflected immediately.
      MethodHandle bound =
          MethodHandles.insertArguments(CHECK_LEVEL, 0, repo, probeName, lo, hi);
  ...
  /**
   * Reads the current level from the repository and returns 1 if it falls within [{@code lo},
   * {@code hi}], or 0 otherwise. Called on every probe entry via the level-check call site.
   */
  public static int checkLevel(HandlerRepository repo, String probeName, int lo, int hi) {
    int level = repo.getLevel(probeName);
    return (level != Integer.MIN_VALUE && level >= lo && level <= hi) ? 1 : 0;
  }

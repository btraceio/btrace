...
public final class BTraceRuntimes {
  ...
  private static boolean loadFactory(String className) {
    try {
      ...
      for (ClassLoader loader : loaders) {
        ...
        try {
          @SuppressWarnings(
              "unchecked") // generic cast due to dynamic classloading of a known API type
  ...
  /**
   * Ensures the runtime accessor is registered in BTraceRuntime. This must be called after
   * getDefault() to complete initialization.
   */
  ...
  /**
   * Drop a runtime previously registered via {@link #getRuntime(String, ArgsMap, CommandListener,
   * java.lang.instrument.Instrumentation)}.
   *
   * <p>This releases the internal registry's strong reference to the {@link BTraceRuntime.Impl} so
   * its class, defining loader, and cached method handles can be collected. Required for any caller
   * that creates a runtime and then aborts before attaching it through the normal lifecycle;
   * without it the registry leaks one entry per aborted attach.
   */
  ...
  /**
   * Get the current instrumentation level for the active BTrace probe. Used by MethodHandle-based
   * level guards to query the level at handler invocation time. Returns 0 if no runtime is active.
   */

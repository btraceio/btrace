...
final class MethodVerifier extends StackTrackingMethodVisitor {
  ...
  public void visitInvokeDynamicInsn(String name, String desc, Handle handle, Object... args) {
    if (InstrumentUtils.isMethodRef(desc, handle)) {
      String target = ((Handle)args[1]).getOwner();
      if (!target.equals(className)) {
        Verifier.reportError("illegal.method.ref", args[1].toString());
      }
    }
      ...
      switch (opcode) {
        ...
        case INVOKESTATIC:
          if (owner.equals(Constants.SERVICE)) {
            ...
            } else if (owner.equals("java/util/Objects")) {
              // This is usually generated by Javac - benign and we should allow it
            } else {
              Verifier.reportError("no.method.calls", owner + "." + name + desc);
            }
          }

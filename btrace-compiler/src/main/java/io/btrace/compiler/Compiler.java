...
public class Compiler {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Compiler compiler = new Compiler(generatePack);
    ...
    try {
      for (File file : files) {
        ...
        preprocessedCompUnits.add(
            MemoryJavaFileManager.makeStringSource(file.getName(), injectDslImport(source)));
      }
  ...
  /**
   * Compile probe from text
   *
   * @param fileName synthetic file name
   * @param source the probe source
   * @param err the error writer
   * @param sourcePath the source path
   * @param classPath the class path
   * @return @return map of generated bytecode data or {@literal null} when compilation fails
   */
  public Map<String, byte[]> compile(
    ...
    compUnits.add(MemoryJavaFileManager.makeStringSource(fileName, injectDslImport(source)));
    return compile(manager, compUnits, err, sourcePath, classPath);
  }

  /**
   * Compile the probe scripts
   *
   * @param compUnits the compilation units
   * @param err error writer
   * @param sourcePath source path
   * @param classPath class path
   * @return map of generated bytecode data or {@literal null} when compilation fails
   */
  public Map<String, byte[]> compile(

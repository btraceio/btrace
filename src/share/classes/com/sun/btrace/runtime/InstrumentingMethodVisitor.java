...
class InstrumentingMethodVisitor extends MethodVisitor implements MethodInstrumentorHelper {
    ...
    private static final class SavedState {
        ...
        private final int nextMappedVar;
        private final boolean reset;

        SavedState(LocalVarTypes lvTypes, SimulatedStack sStack, Collection<LocalVarSlot> newLocals, int firstLocal) {
            this(lvTypes, sStack, newLocals, firstLocal, false);
        }

        SavedState(LocalVarTypes lvTypes, SimulatedStack sStack, Collection<LocalVarSlot> newLocals, int firstLocal, boolean reset) {
    ...
    public void visitJumpInsn(int opcode, Label label) {
        ...
        jumpTargetStates.put(label, new SavedState(localTypes, stack, newLocals, nextMappedVar, opcode == Opcodes.GOTO || opcode == Opcodes.JSR));
    }
        ...
        switch (opcode) {
            ...
            case Opcodes.AALOAD: {
                ...
                if (target instanceof String) {
                    Type t = Type.getObjectType((String)target);
    ...
    public void visitLabel(Label label) {
        ...
        if (handler != null) {
            if (!jumpTargetStates.containsKey(handler)) {
                jumpTargetStates.put(handler, new SavedState(localTypes, stack, newLocals, nextMappedVar, true));
            }
        }
        ...
        if (ss != null) {
            ...
            nextMappedVar = Math.max(ss.nextMappedVar, nextMappedVar);
        }
    ...
    public final void insertFrameReplaceStack(Label l, Type ... stackTypes) {
        ...
        Object[] localsArr = localTypes.toArray();
        ...
        Object[] stackSlots = stack.toArray();
    ...
    private Object[] computeFrameLocals() {
        ...
        if (nextMappedVar > argsSize) {
            int arrSize = newLocals.isEmpty() ? locals.size() : nextMappedVar;
    ...
    private int remap(int var, Type t) {
        return remap(var, t.getSize());
    }
    ...
    private int remap(int var, int size) {
        ...
        if (idx >= 0) {
            if (mapping.length <= idx) {
                ...
                mappedVar = mapping[idx];
            }
            if (mappedVar >= 0) {
//                if (var >= nextMappedVar) {
//                    nextMappedVar = var + size;
//                    var = -var;
//                } else {
                    var = newVarIdx(size);
//                }
        ...
        return mappedVar == Integer.MIN_VALUE ? 0 : Math.abs(mappedVar);
    }
    ...
    private Object[] popFromStack(Type t) {
        Object[] ret = new Object[t.getSize()];
        ret[0] = stack.pop();
        if (t.getSize() == 2) {
            ret[1] = stack.pop();
        }
    ...
    private Type fromSlotType(Object slotType) {
        ...
        return Type.getObjectType((String)slotType);
    }

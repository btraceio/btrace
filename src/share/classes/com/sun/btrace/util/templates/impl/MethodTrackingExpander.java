...
public class MethodTrackingExpander extends BaseTemplateExpander {
     ...
     * <li>{@code $LEVEL=<level>} - the instrumentation intrusiveness level</li>
     * <li>{@code $LEVEL_COND=[EQ, LE, LT, GE, GT]} - the condition to apply to the intrusiveness level (default GE)</li>
     * </ul>
     */
    ...
    public static final String $LEVEL_COND = "level_cond";
    ...
    protected void recordTemplate(Template t) {
        if (ENTRY.equals(t)) {
            ...
            String levelCond = m.get($LEVEL_COND);

            if (levelStr != null) {
                int level = Integer.parseInt(levelStr);
                Interval itv = null;
                Level.Cond cond = Enum.valueOf(Level.Cond.class, levelCond);
                switch (cond) {
                    case EQ: {
                        itv = Interval.eq(level);
                        break;
                    }
                    case GE: {
                        itv = Interval.ge(level);
                        break;
                    }
                    case GT: {
                        itv = Interval.gt(level);
                        break;
                    }
                    case LE: {
                        itv = Interval.le(level);
                        break;
                    }
                    case LT: {
                        itv = Interval.lt(level);
                        break;
                    }
                }
                if (itv != null) {

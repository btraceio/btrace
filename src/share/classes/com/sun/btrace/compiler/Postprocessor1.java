...
import com.sun.btrace.shared.MethodSerializer;
...
final class Postprocessor1 {
    ...
    private static final Type ONMETHOD_TYPE = Type.getType(com.sun.btrace.annotations.OnMethod.class);
    ...
    private final CallGraph1 cGraph = new CallGraph1();
    ...
    private void preprocessMethod(ClassNode cn, MethodNode mn) {
        if (mn.visibleAnnotations != null) {
            for (AnnotationNode an : ((List<AnnotationNode>)mn.visibleAnnotations)) {
                if (an.desc.startsWith("L" + ANNOTATIONS_PREFIX)) {
                    cGraph.addStarting(new CallGraph1.Node(CallGraph1.methodId(mn.name, mn.desc)));
        ...
        try {
            byte[] methodData = MethodSerializer.serialize(mn);
            System.out.println("*** " + methodData.length);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void scanMethodInstructions(ClassNode cn, MethodNode mn, LocalVarGenerator lvg) {
        ...
        for (AbstractInsnNode n = l.getFirst(); n != null; n = n != null ? n.getNext() : null) {
            ...
            if (checkFields && type == AbstractInsnNode.FIELD_INSN) {
                ...
                cGraph.addEdge(CallGraph1.methodId(mn.name, mn.desc), CallGraph1.methodId(min.name, min.desc));
